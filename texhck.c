...
static void texhckCompressDXT1RGB(
      int channels,
      const unsigned char *uncompressed,
      unsigned char compressed[8]);
static void texhckCompressDXT5RGBA(
      const unsigned char *uncompressed,
      unsigned char compressed[8]);
      ...
      unsigned int width, unsigned int height, int channels)
{
   ...
   for (j = 0; j < height; j += 4) {
      for (i = 0; i < width; i += 4) {
         ...
         /* compress the block */
         ++block_count;
         texhckCompressDXT1RGB(3, ublock, cblock);
      ...
      unsigned int width, unsigned int height, int channels)
{
   ...
   for (j = 0; j < height; j += 4) {
      for (i = 0; i < width; i += 4) {
         ...
         texhckCompressDXT5RGBA(ublock, cblock);
         for (x = 0; x < 8; ++x) out[index++] = cblock[x];

         ++block_count;
         texhckCompressDXT1RGB(4, ublock, cblock);
         for (x = 0; x < 8; ++x) out[index++] = cblock[x];
      }
   }
   return 1;
